...
for commodity in commodities:
    ...
    for target in targets:
        ...
        # Model architecture
        model = Sequential([
            LSTM(100, input_shape=(time_steps, X_train_scaled.shape[1]), return_sequences=True),
            Dropout(0.2),
            LSTM(50),
            Dropout(0.2),
        ...
        model.fit(train_generator, epochs=100, validation_data=test_generator, callbacks=callbacks, verbose=1)
        ...
        if model_type == 'regression':
            results_df = pd.DataFrame({'Price': actual_values, 'Pred_Price': predictions.flatten()},
                                      index=adjusted_dates)
        else:
            actual_labels = [data.loc[date, target] for date in
                             adjusted_dates]  # Retrieving original categorical labels
            results_df = pd.DataFrame({'Dir': actual_labels, 'Pred_Dir': predictions},
                                      index=adjusted_dates)
